...
class APIError(Exception):
    def __init__(self, jsonData : dict):
        ...
        message = f"{jsonData["error"]["name"]}: {jsonData["error"]["description"]}, due to {'\n'.join(errors)}"    
...
class BlueFTController:
    def __init__(self, ip: str, mixing_chamber_channel_id: int, port: int = 49099, key: str = None ):
        self.ip = ip
        self.key = key
        self.port = port        
    ...
    def set_mxc_heater_power(self, power: float) -> bool:
        ...
        if power < 0 or power > 1000:
            raise PIDConfigException("Power should be in the range of 0 to 1000 microwatts")
